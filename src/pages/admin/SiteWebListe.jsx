...
export default function SiteWebListe() {
    const [data, setData] = useState([]);
    const token = localStorage.getItem('user_token')

    useEffect(() => {
        axios.get(`${API_URL}creation-site-web`,{
            headers: {
          ...
          <tbody>
           {
            data.map((item,index) => (
                <tr key={index}>
                    <td><img src={`${API_STORAGE}${item.image}`} height={100} width={100} alt="image"  /></td>
                    <td>{item.lien}</td>
                    <td>
                        <Link to={`/dashboard/site-web/modifier/${item.id}`} className='btn btn-success'>modifier</Link>
                    </td>
                </tr>
            ))
           }
